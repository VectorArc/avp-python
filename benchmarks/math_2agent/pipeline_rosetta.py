...
def run_rosetta_pipeline(
    ...
    """Run the 2-agent cross-model pipeline on a single MATH problem.
    ...
    with gpu_memory_tracker(device) as mem:
        ...
        messages = build_latent_prompt(researcher.role, problem)
        ...
        messages = build_latent_prompt(solver.role, problem)
...
def run_rosetta_benchmark(
    ...
    """Run rosetta-mode pipeline on a list of MATH samples."""
    results = []
    for i, sample in enumerate(dataset):
        if verbose:
            print(f"\n[Rosetta] Sample {i + 1}/{len(dataset)}: {sample['problem'][:80]}...")
