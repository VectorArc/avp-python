...
Rosetta beats text on code generation (HumanEval) in both directions. On math (GSM8K), text wins when the weaker model solves, rosetta wins when the stronger model solves. Vocabulary-mediated projection – no learned parameters, works across model families.
